...
- `request_timeout`;
- `directum_rules_path`;
- необязательные `dadata_api_key`, `dadata_timeout`, `dadata_party_url`.
...
4. если корреспондент не найден и в данных есть ИНН, получает название
   компании через DaData и повторяет поиск в `ICounterparties`;
5. применяет правила уровня Directum ID;
6. при необходимости пересылает письмо и пропускает создание документа;
7. формирует payload для `IIncomingLetters`;
8. загружает PDF-вложения как версии документа;
9. создает задачу на проверку, если были ошибки сопоставления.
10. если ошибок сопоставления не было, создает простую задачу с просьбой
...
## Резервный поиск контрагента через DaData

Если Directum не нашел контрагента по названию из AI-результата, а поле `inn`
заполнено, клиент отправляет ИНН в DaData.ru `findById/party`. Из ответа
берется `short_with_opf`, затем `full_with_opf`, затем `value`. Полученное
название используется только для повторного поиска в `ICounterparties`.

DaData не используется в обычном успешном сценарии и не создает контрагента в
Directum. Если `dadata_api_key` не указан, fallback пропускается.
...
не удалось сопоставить уверенно. Письмо считается частично обработанным, если
любой JSON-поле для карточки письма отсутствует или пришло пустой строкой:
`correspondent`, `dateFrom`, `number`, `signedBy`, `recipient`. `inn` является
вспомогательным полем и не делает письмо частично обработанным. Пустые
значения превращаются в ошибки проверки, даже если документ Directum был создан.
...
## Пересылка в приемную получателя

ID-level правила могут сработать после AI и поиска получателя в Directum. Если
`recipient_id` совпадает с правилом приемной, приложение пересылает исходное
письмо на email приемной, не создает документ Directum и возвращает результат
`forwarded_to_recipient=True`. Такой маршрут учитывается отдельным счетчиком
статистики `forwarded_recipient`.
